...
module.exports = router
    ...
    .get('/created-cards', ensureAuth, (req, res, next) => {
        Flashcard.find({ profileId: req.user.profileId, deleted: false })
            .lean()
            .then(flashcards => {
                res.json(flashcards);
            })
    ...
    .delete('/:id', ensureAuth, (req, res, next) => {
        Flashcard.findById(req.params.id)
            .lean()
            .then(flashcard => {
                if(req.user.profileId !== flashcard.profileId.toString()) {
                    next(new HttpError({
                        code: 403,
                        message: 'Forbidden - Not the owner of this resource'
                    }));
                    ...
                    Flashcard.findByIdAndRemove(req.params.id)
                        .then(flashcard => res.json({ removed: !!flashcard }))
                        .catch(next);
                }
            });
    });